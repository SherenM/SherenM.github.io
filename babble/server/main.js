...
var server = http.createServer(function (request, response) {
    ...
    /*
        url_parts.hostname ---> localhost:9000
        url_parts.pathname ---> after 9000 (/filename)
        url_parts.search   ---> all after filename (the query)
    */
    ...
    if (method === 'POST') {
            ...
            request.on('data', (chunk) => {
                ...
                while (statsReq.length > 0) {
                    ...
                    var arr = {
                        ...
                        messages: messages.Babble.messagesList.length
                    };
        ...
        else if (path === "/messages")     //this garantee also that pathname="/messages"
        {
            //NO use of express.parser...
            //'data' and 'end' are streams to listen to
            postBody = [];
            request.on('data', (chunk) => {
                postBody.push(chunk);
            }).on('end', () => {
                ...
                var msgId = messages.addMessage(messageObject);
                ...
                while (msgsReq.length > 0) {
                    ...
                    client.res.end(JSON.stringify(arr));
                }
                var connectedUsers = countConnectedUsers();
                while (statsReq.length > 0) {
                    var st = statsReq.pop();
            ...
            request.on('data', (chunk) => {
                ...
                if (parsedData.name === undefined || parsedData.email === undefined || parsedData.name === "" || parsedData.email === "") {
                    ...
                    messages.Babble.usersList.push(userInfo);
                }
                else {
                    //check if there a user with the same details that was registered before
                    ...
                    if (userIndex >= 0) {
                        ...
                        currentMsg = messages.Babble.usersList[userIndex].currentMessage;
                    } else {
                        //register the new user
                        userInfo = {
                            name: parsedData.name,
                            email: parsedData.email,
                            gravatar: 'http://www.gravatar.com/avatar/' + MD5(parsedData.email) + '.png?d=identicon',
                            connected: 1,
                            currentMessage: ""
                        };
                        messages.Babble.usersList.push(userInfo);
        ...
        if (pathname === ("/messages/" + message_id)) {
            ...
            if (isNaN(message_id)) {
                ...
                if (message_id <= messages.Babble.serverCounter) {
                    //delete message by id
                    messages.deleteMessage(message_id);  //message_id sent must be string
                    var connectedUsers = countConnectedUsers();
                    while (statsReq.length > 0) {
                        var st = statsReq.pop();
                        var arr = {
        ...
        if (path === '/stats') {
            ...
            if (strCounter != "counter" || strEqual != "=" || isNaN(counterVal) || rest != "") {
                ...
                if (counterVal < messages.Babble.lastMsgId) {
                    ...
                    msgsReq.push({ res: response, timestamp: new Date().getTime() });
                }
...
setInterval(function () {
    // close out requests older than 60 seconds
    var expiration = new Date().getTime() - 60000;
    var response;
    for (var i = msgsReq.length - 1; i >= 0; i--) {
        if (msgsReq[i].timestamp < expiration) {
            response = msgsReq[i].res;
            response.statusCode = 200;
            response.end("");
        }
    }

    for (var j = statsReq.length - 1; j >= 0; j--) {
        if (statsReq[j].timestamp < expiration) {
            response = statsReq[j].res;
            response.statusCode = 200;
            response.end("");
        }
    }
...
function countConnectedUsers() {
    var arr = [];
    arr = messages.Babble.usersList.filter(function (element) {
        return element.connected === 1;
    });
    return arr.length;
}
